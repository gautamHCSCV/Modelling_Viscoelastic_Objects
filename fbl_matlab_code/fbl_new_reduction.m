...
for i = 1:9
    ...
    g6k = -2*imag(gk(4))/length(fk);
    g7k = 2*real(gk(5))/length(fk);
    g8k = -2*imag(gk(5))/length(fk);
    g9k = 2*real(gk(6))/length(fk);
...
g0 = horzcat(y0,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10);
...
wi = (w - min(w))/(max(w) - min(w));
ampi = (amp - min(amp))/(max(amp) - min(amp));


D = horzcat(ampi,wi);


csvwrite('aaa_out_gk_dragon.csv', g0);
csvwrite('aaa_inp_z_dragon.csv', D);
