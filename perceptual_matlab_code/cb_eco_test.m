...
F = [];
P = 'C:\Users\DELL\Downloads\Homogeneous_data_Access\Ecoflex_homogeneous_Sinusoidal_220414_validation';
...
for i = 1:4
    ...
    t = f(i,1:77);
    m = 'C:\Users\DELL\Downloads\Homogeneous_data_Access\Ecoflex_homogeneous_Sinusoidal_220414_validation\';
    y = append(m,t);
    Spill1 = importdata(y);
    x = Spill1(:,4);
    y = Spill1(:,16);
    ...
    vel1 = diff(x);
    velocity1_z=[vel1(1); vel1];
    fc1=25;
    fs=2000;
    [b1,a1]=butter(1,fc1/(fs/2));
    filtered_velocity1_z_25=filter(b1,a1,velocity1_z);

    f1 = [f1;y];
    dis1 = [dis1;x];
    data_v1 = [data_v1;velocity1_z];
    data_fv1 = [data_fv1;filtered_velocity1_z_25];
...
for i = 1:2
    ...
    in = weber_sampling(y,0.05);
    B = reshape(in,[length(in),1]);
    res = [res;B];


    %v1 = zeros( length(in), length(order));
    %for i = 1:length(order)
    %yd =  fderiv(x, 1, 180, 0.001);
    %v1 = reshape(yd,[length(yd),1]);
    %end

    %B = zeros( length(in), length(order));
    ...
    for j = 1:length(in)
        data_v1 = [data_v1;velocity1_z(in(j),:)];
        
    end

    for j = 1:length(in)
        data_fv1 = [data_fv1;filtered_velocity1_z_25(in(j),:)];
...
D1 = horzcat(f1);
D2 = horzcat(dis1,data_v1,data_fv1);

csvwrite('force_percep_cb_test_ecoflex.csv', D1);
csvwrite('pos_percep_cb_test_ecoflex.csv', D2);
...
function [ind]=weber_sampling(f,thr)
Ref=f(1);
ind=[];
for j=2:length(f)
    a=Ref;
