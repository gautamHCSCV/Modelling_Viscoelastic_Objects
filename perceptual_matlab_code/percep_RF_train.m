...
B = zeros( length(dis1), length(order));
for i = 1:length(order)
     yd = fderiv( dis1,order(i), 180,h );
     B(:,i) = reshape(yd,[length(yd),1]);
end 
...
csvwrite('force_percep_rf_train_dragon.csv', D1);
csvwrite('pos_percep_rf_train_dragon.csv', D2);
