...
F = [];
P = 'C:\Users\DELL\Downloads\Homogeneous_data\Dragonskin_homogeneous_Sinusoidal_210608_validation';
...
for i = 1:10
t = F(i,1:77);
m = 'C:\Users\DELL\Downloads\Homogeneous_data\Dragonskin_homogeneous_Sinusoidal_210608_validation\';
y = append(m,t);
Spill1 = importdata(y);
x = Spill1(:,4);
y = Spill1(:,16);
force = [force;y];
pos = [pos;x];
%B = zeros( length(x), length(order));
%for i = 1:length(order)
yd = fderiv( x,1, 180,1 );
B = reshape(yd,[length(yd),1]);
...
csvwrite('imp_force_percp_test_upd.csv', force);
csvwrite('imp_pos_percep_test_upd.csv', D);
